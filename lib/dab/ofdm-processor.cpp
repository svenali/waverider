 ...
 *    (Modification in method restart(). There is a difference between ThreadHandling in Wt)
 *
 *    Copyright (C) 2018
...
void OFDMProcessor::restart()
{
    ...
    /**
     * (c) Dr. Sven Alisch 
     * das geht hier nur, wenn als GUI Qt zum Einsatz kommt.
     * Für Wt muss der Thread deatached werden, d. h. ja, man hat leider keine 
     * Kontrolle mehr darüber. Durch setzen von running=false, und 500ms warten sollte es jedoch
     * dennoch gehen. 
     *
    if (threadHandle.joinable()) {
    ...
    */
    ...
    // Gleich detach und damit kollidieren HauptThread von Wt nicht mit dem OFDM Prozessor.
    threadHandle.detach();
}
...
void OFDMProcessor::stop()
{
    if (running) {
        running = false;
        if (threadHandle.joinable()) {
            std::cout << "Das kommt noch oder?" << std::endl;
            threadHandle.join();
            std::cout << "Das kommt nicht mehr oder?" << std::endl;
        }
    }
}
