 ...
 */
#ifndef _CAACAUDIOCOMPRESSION_H_
#define _CAACAUDIOCOMPRESSION_H_
...
struct TAACBufferConf {
        RingBuffer<uint8_t> *exportBuffer;
        uint8_t *pointerToIndividualBuffer;     // custom output_buffer for a later free
};

static int writeAACToRingBuffer(void* opaque, uint8_t* buf, int buf_size);
...
class CAACAudioCompression : public CAudioCompression
{
    public:
        CAACAudioCompression(CRadioController* r);
        CAACAudioCompression(CRadioController* r, string filename);
        virtual ~CAACAudioCompression();
...
#endif
        void encodeFrame();
        TAACBufferConf _externBufferConf;
};
