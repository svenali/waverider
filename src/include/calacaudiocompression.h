 ...
 */
#ifndef _CALACAUDIOCOMPRESSION_H_
#define _CALACAUDIOCOMPRESSION_H_
...
struct TALACBufferConf {
        RingBuffer<uint8_t> *exportBuffer;
        uint8_t *pointerToIndividualBuffer;
};

static int writeALACToRingBuffer(void* opaque, uint8_t* buf, int buf_size);
static int64_t seekALACInBuffer(void *opaque, int64_t offset, int whence);

class CALACAudioCompression : public CAudioCompression
{
    public:
        CALACAudioCompression(CRadioController* r);
        CALACAudioCompression(CRadioController* r, string filename);
        virtual ~CALACAudioCompression();
    ...
    private:
        ...
        TALACBufferConf _externBufferConf;
};
