 ...
 */
#ifndef _CFLACAUDIOCOMPRESSION_H_
#define _CFLACAUDIOCOMPRESSION_H_
...
struct TIndividualBufferConf {
        RingBuffer<uint8_t> *exportBuffer;
        uint8_t *pointerToIndividualBuffer;
};

static int writeToRingBuffer(void* opaque, uint8_t* buf, int buf_size);

class CFLACAudioCompression : public CAudioCompression
{
    public:
        CFLACAudioCompression(CRadioController* r);
        CFLACAudioCompression(CRadioController* r, string filename);
        virtual ~CFLACAudioCompression();
    ...
    private:
        ...
        TIndividualBufferConf _externBufferConf;
};
