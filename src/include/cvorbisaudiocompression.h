 ...
 */
#ifndef _CVORBISAUDIOCOMPRESSION_H_
#define _CVORBISAUDIOCOMPRESSION_H_
...
struct TVORBISBufferConf {
        RingBuffer<uint8_t> *exportBuffer;
        uint8_t *pointerToIndividualBuffer;     // custom output_buffer for a later free
};

static int writeVORBISToRingBuffer(void* opaque, uint8_t* buf, int buf_size);

class CVORBISAudioCompression : public CAudioCompression
{
    public:
        CVORBISAudioCompression(CRadioController* r);
        CVORBISAudioCompression(CRadioController* r, string filename);
        virtual ~CVORBISAudioCompression();
...
#endif
        void encodeFrame();
        TVORBISBufferConf _externBufferConf;
};
